...
class LocalWorkflowDefinitionClientTest {
    ...
    @Test
    void testGetWorkflowDelegate() {
        String defId = "wf-1";
        when(definitionService.get(any(), any())).thenReturn(Uni.createFrom().item(mockDefinition));

        WorkflowDefinition response = client.getWorkflow(defId).await().indefinitely();
    ...
    @Test
    void testListWorkflowsDelegate() {
        when(definitionService.list(any(), anyBoolean()))
                .thenReturn(Uni.createFrom().item(List.of(mockDefinition)));

        List<WorkflowDefinition> response = client.listWorkflows().await().indefinitely();
