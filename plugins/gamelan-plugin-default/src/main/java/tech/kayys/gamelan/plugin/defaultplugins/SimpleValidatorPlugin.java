...
public class SimpleValidatorPlugin implements WorkflowValidatorPlugin {
    ...
    @Override
    public List<ValidationError> validate(WorkflowValidatorPlugin.WorkflowDefinitionInfo workflowDefinition) {
        if (!started) {
        ...
        if (workflowDefinition.name() == null || workflowDefinition.name().trim().isEmpty()) {
            errors.add(new ValidationError("WORKFLOW_NAME_RULE", "Workflow name is required", "definition.name",
                    ValidationError.Severity.ERROR));
        }

        // Validate workflow has nodes
        if (workflowDefinition.nodes() == null || workflowDefinition.nodes().isEmpty()) {
            errors.add(new ValidationError("NODE_COUNT_RULE", "Workflow must have at least one node",
                    "definition.nodes", ValidationError.Severity.ERROR));
        }

        // Validate that all nodes have unique IDs
        if (workflowDefinition.nodes() != null) {
            List<String> nodeIds = new ArrayList<>();
            for (WorkflowValidatorPlugin.NodeDefinitionInfo node : workflowDefinition.nodes()) {
                if (nodeIds.contains(node.nodeId())) {
                    errors.add(new ValidationError("UNIQUE_ID_RULE", "Duplicate node ID: " + node.nodeId(),
                            "definition.nodes", ValidationError.Severity.ERROR));
                } else {
        ...
        if (workflowDefinition.transitions() != null) {
            for (WorkflowValidatorPlugin.TransitionInfo transition : workflowDefinition.transitions()) {
                if (transition.fromNodeId() == null || transition.toNodeId() == null) {
                    errors.add(new ValidationError("TRANSITION_RULE", "Transition must have both 'from' and 'to' nodes",
                            "definition.transitions", ValidationError.Severity.ERROR));
                }
            }
        }
